    ...
    <form onSubmit={handleSubmit} className="application-form space-y-4 max-h-[70vh] overflow-y-auto pr-1">
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <Input className="rounded-[10px]" value={company} onChange={e => setCompany(e.target.value)} autoFocus />
        </div>
        <div className="space-y-1">
          <Label>Role <span className="text-destructive">*</span></Label>
          <Input className="rounded-[10px]" value={role} onChange={e => setRole(e.target.value)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          ...
          <Input className="rounded-[10px]" type="date" value={appliedDate} onChange={e => setAppliedDate(e.target.value)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <Input className="rounded-[10px]" placeholder="Remote / City, Country" value={location} onChange={e => setLocation(e.target.value)} />
        </div>
        <div className="space-y-1">
          <Label>Salary Range</Label>
          <Input className="rounded-[10px]" placeholder="e.g. ₹8L–₹12L" value={salaryRange} onChange={e => setSalaryRange(e.target.value)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <Input className="rounded-[10px]" placeholder="e.g. v3-product" value={resumeVersion} onChange={e => setResumeVersion(e.target.value)} />
        </div>
        <div className="space-y-1">
          <Label>Company Size</Label>
          <Select value={companySize} onValueChange={setCompanySize}>
            <SelectTrigger className="rounded-[10px]"><SelectValue placeholder="Select size" /></SelectTrigger>
            <SelectContent>
      ...
      <div className="space-y-1">
        <Label>Job URL</Label>
        <Input className="rounded-[10px]" placeholder="https://..." value={jobUrl} onChange={e => setJobUrl(e.target.value)} />
      </div>

      <div className="space-y-1">
        <Label>Follow-up Date</Label>
        <Input className="rounded-[10px]" type="date" value={followUpDate} onChange={e => setFollowUpDate(e.target.value)} />
      </div>

      <div className="space-y-1">
        <Label>Notes</Label>
        <Textarea className="rounded-[10px]" value={notes} onChange={e => setNotes(e.target.value)} rows={3} />
      </div>

      <Button
        type="submit"
        disabled={!isFormValid}
        className={`rounded-[10px] w-full bg-app-accent text-black hover:opacity-90 ${!isFormValid ? "cursor-not-allowed opacity-50" : ""}`}
      >
        Save Application
      </Button>
    </form>
