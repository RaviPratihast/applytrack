...
/**
 * @typedef {Object} KanbanProps
 * @property {Record<import('@dnd-kit/core').UniqueIdentifier, T[]>} value
 * @property {(columns: Record<import('@dnd-kit/core').UniqueIdentifier, T[]>) => void} [onValueChange]
 * @property {(item: T) => import('@dnd-kit/core').UniqueIdentifier} [getItemValue]
 * @property {(event: import('@dnd-kit/core').DragEndEvent & { activeIndex: number; overIndex: number }) => void} [onMove]
 * @property {import('@dnd-kit/sortable').SortableContextProps['strategy']} [strategy]
 * @property {'horizontal' | 'vertical'} [orientation]
 * @property {boolean} [flatCursor]
 * @template T
 */
...
const KanbanBoard = React.forwardRef(function KanbanBoard(props, ref) {
...
const KanbanColumn = React.forwardRef(function KanbanColumn(props, ref) {
  const {
    value,
    asChild,
    asHandle,
...
const KanbanColumnHandle = React.forwardRef(function KanbanColumnHandle(
  props,
  ref
) {
...
const KanbanItem = React.forwardRef(function KanbanItem(props, ref) {
  const {
    value,
    style,
    asHandle,
  ...
  if (!inBoard && !inOverlay) {
    throw new Error(
      `\`${ITEM_NAME}\` must be used within \`${BOARD_NAME}\` or \`${OVERLAY_NAME}\``
    );
  }
...
const KanbanItemHandle = React.forwardRef(function KanbanItemHandle(
  props,
  ref
) {
...
KanbanOverlay.displayName = OVERLAY_NAME;
